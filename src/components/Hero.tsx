export default function Hero() {
  return (
    <section className="flex flex-col justify-center min-h-screen px-10 pt-32 pb-20 border-b border-white/10">
      ...
      <div className="flex items-center gap-2 text-[#2e9e6b] text-xs font-bold uppercase tracking-widest mb-8">
        <span className="w-6 h-[2px] bg-[#2e9e6b]" />
      ...
      <h1 className="text-5xl lg:text-7xl font-black text-white leading-tight tracking-tighter mb-6 max-w-4xl">
        Formez-vous.<br />
        Évoluez.<br />
        <span className="relative inline-block text-[#2e9e6b]">
          Performez.
          <span className="absolute left-0 -bottom-1 w-full h-[3px] bg-[#f97316] rounded-full" />
        </span>
      </h1>

      <p className="text-[#7a96aa] text-base leading-relaxed max-w-xl mb-10">
      ...
      <div className="flex gap-2 flex-wrap">
        {["Photovoltaïque", "Fibre optique", "Télécoms", "Certifiant", "Éco-responsable"].map((tag) => (
          <span
            key={tag}
            className="text-xs font-semibold px-3 py-1 rounded-sm bg-[#2e9e6b]/10 text-[#2e9e6b] border border-[#2e9e6b]/25"
          >
            {tag}
          </span>
        ))}
